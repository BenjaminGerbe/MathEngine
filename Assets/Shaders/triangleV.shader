...
void main(void){
    ...
    vec4 positionNormal = u_localTranslation_mat* rotateVertex(a_normal.xyz,u_localRotation_quat,u_localRotation_conj) ;
    vec4 rotationNormal =   rotateVertex(vec3(positionNormal.xyz),u_globalRotation_quat,u_globalRotation_conj);

    v_normal =  vec4(position.xyz,0.0);
    v_color = a_color;
    v_uv = vec2(a_uv.x,1-a_uv.y);

    mat4 cam =  u_cameraTranslation_mat * u_rotationCamera ;
